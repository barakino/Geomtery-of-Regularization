...
# Removed LaTeX formatting to prevent MathText parsing errors on the server
ax.set_xlabel("Beta 1", fontsize=14)
ax.set_ylabel("Beta 2", fontsize=14)

# 1. Draw Constraint Region
if reg_type == "Ridge (L2 Norm)":
    constraint = plt.Circle((0, 0), t, color='#4A90E2', alpha=0.2, label='Ridge Constraint (L2)')
    ax.add_patch(constraint)
    ax.plot(0, 0, color='#4A90E2', label='_nolegend_')
else:
    diamond = plt.Polygon([[t, 0], [0, t], [-t, 0], [0, -t]], color='#4A90E2', alpha=0.2, label='Lasso Constraint (L1)')
...
# Added a tiny epsilon buffer (1e-4) to prevent contouring a true 0-radius circle
if optimal_loss > 1e-4:
...
# 3. Plot Points (Removed LaTeX formatting from labels)
ax.plot(beta1_hat, beta2_hat, 'ko', markersize=8, label='Unconstrained OLS')
ax.plot(opt_b1, opt_b2, 'ro', markersize=8, label='Optimal Penalized Beta')
...
with col2:
    ...
    st.markdown(f"**Optimal Beta 1:** `{opt_b1:.3f}`")
    st.markdown(f"**Optimal Beta 2:** `{opt_b2:.3f}`")
    
    st.markdown("---")
    
    is_sparse = (np.isclose(opt_b1, 0) or np.isclose(opt_b2, 0)) and optimal_loss > 1e-4
